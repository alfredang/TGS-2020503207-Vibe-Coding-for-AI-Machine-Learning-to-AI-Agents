...
- Let users toggle these regularization techniques on/off via checkboxes:
  ...
  - Batch Normalization
  - Data Augmentation (RandomFlip + RandomRotation, uses CNN architecture)
...
2. **Fashion-MNIST data pipeline** -- download, normalize, train/val/test split, flat + image versions
3. **Baseline model** -- large dense network (512-512-256-256-128) with no regularization
4. **Regularized model builder** -- dynamically adds Dropout, BatchNorm, Augmentation, L2 based on user selections
...
| Change dataset | "Switch from Fashion-MNIST to CIFAR-10 and use a deeper CNN" |
...
| Augmentation only | Augmentation ON | Slower training, better generalization |
...
2. **Each technique targets overfitting differently** -- Dropout prevents co-adaptation, BatchNorm stabilizes training, Augmentation increases effective data, L2 penalizes large weights, Early Stopping halts before overfitting
3. **Combining techniques** usually gives the best results -- they complement each other
4. **The overfitting gap** (train accuracy - val accuracy) is the key metric to watch, not just accuracy
5. **Vibe coding** lets you build complex ML comparison tools by describing what you want and iterating
